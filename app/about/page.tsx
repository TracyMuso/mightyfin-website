  ...
  return (
    <div>
      ...
      <section className="lg:px-16 sm:px-12 px-8 py-10 md:h-[100vh] md:relative">
        <h1 className="lg:text-4xl sm:text-2xl pb-3 text-xl text-purple-600">
        ...
        <div className="p-4 rounded-md bg-gray-50 md:text-left md:absolute left-24 bottom-12 sm:w-[500px]">
          ...
          <p className="font-light pt-3">
            Many people struggle with confusing loan applications, hidden fees,
            and inflexible repayment options. Mighty Fin simplifies the process
            with an easy-to-use platform, clear rates, and personalized,
            flexible repayment plans, making borrowing straightforward and
            stress-free.
          </p>
        </div>
      </section>
      ...
      <div className="bg-yellow-500 flex flex-col gap-5 items-center xl:h-[100vh] px-10 py-16">
        ...
        <div className="flex md:flex-row md:flex-wrap xl:flex-nowrap flex-col items-center xl:gap-12 lg:gap-5 md:gap-1 gap-8">
          {featuresData.map((item, idx) => (
            <div
              key={idx}
              className="w-[340px] h-[400px] rounded-md px-4 py-6 flex flex-col items-center gap-3 bg-white"
            >
