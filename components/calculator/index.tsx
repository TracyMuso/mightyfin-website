...
const LoanCalculator: React.FC = () => {
  ...
  const [selectedLoanType, setSelectedLoanType] = useState<LoanType>('Civil servant loan');
  ...
  const handleLoanTypeChange = (e: React.ChangeEvent<HTMLSelectElement>): void => {
  ...
  return (
    <div className={styles.calculatorContainer}>
      <h3 className='text-purple-700 font-semibold py-5 md:text-3xl text-xl'>Pick your loan!</h3>
      ...
      <div className={styles.results}>
        <div className={styles.resultItem}>
          ...
          <span>{calculateInterestRate(loanTerm)}%</span>
        </div>
        <div className={styles.resultItem}>
          <span>Monthly Repayment:</span>
          <span>ZMW {monthlyRepayment.toFixed(2)}</span>
        </div>
        <div className={styles.resultItem}>
        ...
        <div className={styles.resultItem}>
          ...
          <span>ZMW {totalRepayment.toFixed(2)}</span>
           ...
           <div className={styles.resultItem}>
            ...
            <span>{getNextRepaymentDate()}</span>
        </div>
        <Button variant='primary' value='Apply Now!' />
      </div>
    </div>
  );
};
