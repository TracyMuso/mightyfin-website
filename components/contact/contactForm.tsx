      ...
      <p className="md:text-[17px] text-m py-2 w-full text-purple-600 font-bold">Please fill in all the fields</p>
        ...
        {FormFieldData.map((data, idx) => (
            <div key={idx} className="flex flex-col">
              <label htmlFor={data.for} className="mb-2 text-black text-sm sm:text-base">{data.label}</label>
               <Input id={data.id} name={data.name} placeholder={data.placeholder} type={data.type} />
           </div>  
          ))}
        ...
        <button className="w-full bg-purple-600 text-white px-4 py-3 rounded-md" type="submit">Send Message</button>
      </form>
  )
}
